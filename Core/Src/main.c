...
// Variáveis para armazenar o Duty Cycle atual (0 a 100)
volatile uint8_t duty_r = 0;
volatile uint8_t duty_g = 0;
volatile uint8_t duty_b = 0;

// Flag de atualização do display
...
int main(void)
{
  ...
  // Zera os duty cycles inicialmente
  ...
  // Inicia a recepção UART por interrupção (1 byte por vez)
  HAL_UART_Receive_IT(&huart5, &rx_byte, 1);
  /* USER CODE END 2 */
